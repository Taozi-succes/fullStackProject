...
const { HTTP_STATUS, RESPONSE_MESSAGES, ERROR_CODES } = require('../../../common/constants');
...
class UserService {
  ...
  async login(loginData) {
    try {
      ...
      // 验证验证码
      const captchaResult = await this.captchaService.verifyCaptcha(captchaId, captchaCode);
      ...
      if (user.status !== UserStatusEnum.ACTIVE) {
        logger.warn('登录失败：用户账户被禁用', { userId: user.id, status: user.status });
        return {
          success: false,
  ...
  async register(registerData) {
    try {
      const { username, email, password, captchaId, captchaCode } = registerData;
  ...
  async updateUser(userId, updateData) {
    try {
      ...
      if (updateData.username && updateData.username !== existingUser.username) {
        const userWithSameUsername = await this.findUserByUsername(updateData.username);
