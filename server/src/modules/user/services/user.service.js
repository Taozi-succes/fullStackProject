...
class UserService {
            ...
            const result = await this.prisma.$transaction(async (prisma) => {
                ...
                if (currentUser.avatar) {
                    ...
                    const existingHistory =
                        await prisma.avatarHistory.findFirst({
                            where: {
                                userId: userId,
    ...
    async getAvatarHistory(userId) {
        try {
            ...
            return {
                ...
                data: {
                    current:
                        history.find((h) => h.isCurrent)?.avatarUrl || null,
                    history: history
    ...
    async cleanOrphanAvatars() {
        try {
            ...
            users.forEach((user) => {
                if (user.avatar) {
                    const filename = user.avatar.replace(
                        "/uploads/avatars/",
                        ""
                    );
            ...
            avatarHistory.forEach((history) => {
                if (history.avatarUrl) {
                    const filename = history.avatarUrl.replace(
                        "/uploads/avatars/",
                        ""
                    );
                    usedAvatars.add(filename);
